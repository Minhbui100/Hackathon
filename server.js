...
app.put("/bill/:bill_id", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "UPDATE bill SET cust_id = $1, tip=$2, card_id = $3, paid = TRUE WHERE bill_id = $4",
      [customerId, tip, cardId, bill_id]
    );
    ...
    await pool.query(
      "UPDATE customers SET membership_point = membership_point + 1 WHERE id = $1",
      [customerId]
    );
    ...
    await pool.query(
      `INSERT INTO transaction (bill_id, total, from_bankacct, bussiness_balance) 
             VALUES ($1, $2, $3, $4)`,
      [bill_id, formattedTotalAmount, cardId, newcurrentBusinessBalance]
    );
